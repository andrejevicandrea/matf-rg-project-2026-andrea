...
namespace engine::graphics {
    /**
        ...
        */
    struct MultisampleFramebufferData {
        uint32_t framebuffer_id;
        uint32_t color_texture_id;
        uint32_t depth_stencil_renderbuffer_id;
    };

    struct PointShadowFramebufferData {
        uint32_t framebuffer_id   = 0;
        uint32_t depth_cubemap_id = 0;
    };
    ...
    public:
        ...
        static MultisampleFramebufferData initialize_multisample_framebuffer(
            int32_t width, int32_t height, int32_t samples);

        static PointShadowFramebufferData initialize_point_shadow_framebuffer(int32_t width, int32_t height);

        static TexturedQuadData initialize_textured_quad(const float *vertices,
